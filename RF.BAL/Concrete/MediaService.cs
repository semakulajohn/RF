  ...
  public  class MediaService :IMediaService
    {
        ...
        public RF.Models.Media GetMedia(long mediaId)
        {
        ...
        private Media FilterByMediaTypes(Media media, string filterMediaTypes)
        {
            if (!string.IsNullOrEmpty(filterMediaTypes))
            {
                ...
                {
                    var filteredChildren = new List<RF.Models.Media>();
        ...
        private Media MapEFToModel(EF.Models.Media m)
        {
            if (m != null)
            {
                ...
                if (children != null)
                {
                    var list = new List<RF.Models.Media>();
                    media.Children = list;
        ...
        public RF.Models.MediaFolder GetFolderAndChildFolders(long parentFolderId)
        {
            var m = _dataService.GetMedia(parentFolderId);
            var children = _dataService.GetChildFolders(parentFolderId);
            var media = new RF.Models.MediaFolder
            {
                id = m.MediaId,
                MediaId = m.MediaId,
                MediaGuid = m.MediaGuid,
            ...
            if (children != null)
            {
                var list = new List<RF.Models.MediaFolder>();
                media.children = list;
                foreach (var child in children)
                {
                    media.children.Add(
                        new RF.Models.MediaFolder
                        {
                            id = child.MediaId,
                            MediaId = child.MediaId,
                            ParentId = child.ParentId,
        ...
        public IEnumerable<RF.Models.MediaFolder> GetChildFolders(long parentId)
        {
        ...
        public IEnumerable<RF.Models.Media> GetFilesInFolder(long folderId, string filterMediaTypes)
        {
            var allowedMediaTypesIds = filterMediaTypes.Split(',').Select(Int64.Parse).ToList();
        ...
        public IEnumerable<RF.Models.Media> GetFilesInFolder(long folderId)
        {
            var children = _dataService.GetFilesInFolder(folderId);
            var list = new List<RF.Models.Media>();
            foreach (var m in children)
        ...
        public IEnumerable<RF.Models.ExtensionType> GetAllExtentionTypes()
        {
        ...
        public IEnumerable<MediaFolder> DrillDownToMediaId(long mediaId)
        {
            ...
            var rootFolder = new List<RF.Models.MediaFolder>();
        ...
        private List<RF.Models.MediaFolder> getChildFoldersToAdd(ref List<long> ids, long parentMediaId)
        {
