...
namespace RF.DAL.Concrete
{
  public  class MediaDataService : DataServiceBase,IMediaDataService
    {
        public MediaDataService(IUnitOfWork<RentalFinderEntities> unitOfWork)
            : base(unitOfWork)
        {
        ...
        {
            return (from n in this.UnitOfWork.Get<Media>().AsQueryable()
                    where n.ParentId == parentId && n.MediaTypeId == (long)RF.Library.EnumTypes.MediaType.Folder && (n.Deleted == null || !n.Deleted)
                    select n);
        }
        ...
        {
            return (from n in this.UnitOfWork.Get<Media>().AsQueryable()
                    where n.ParentId == folderId && n.MediaTypeId != (long)RF.Library.EnumTypes.MediaType.Folder && (n.Deleted == null || !n.Deleted)
                    select n);
        }
        ...
        public IEnumerable<MediaDTO> GetDescendants(long mediaId)
        {
            using (var dbContext = new RentalFinderEntities())
            {
                ...
                Mapper.CreateMap<RF.EF.Models.Media_GetDescendants_Result, RF.DTO.MediaDTO>();
                return Mapper.Map<List<RF.EF.Models.Media_GetDescendants_Result>, List<RF.DTO.MediaDTO>>(results);
            }
        }
