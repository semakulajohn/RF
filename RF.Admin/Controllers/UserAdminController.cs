...
namespace RF.Admin.Controllers
{
    ...
    {
        RentalFinderEntities context = new RentalFinderEntities();
        ...
        public ActionResult LandLords(string sortingOrder, string searchData, string Filter_Value, int? Page_No, string submit = "")
        {
            int Size_Of_Page = PAGESIZE;
            ViewBag.CurrentSortOrder = sortingOrder;
            ViewBag.SortingFirstName = String.IsNullOrEmpty(sortingOrder) ? "firstNameSorting" : "";
            ...
            var usersx = UserManager.Users.ToList();
            var users = _userService.GetAllLandLords();
            if (users != null)
            {
                users = users.Where(u => u.FirstName.ToLower().Contains(searchText) ||
                u.LastName.ToLower().Contains(searchText)
